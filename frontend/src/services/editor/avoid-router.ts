...
export class AvoidRouter {
  ...
  shapeRefs: { [key: dia.Cell.ID]: any }; // [element.id]: shapeRef

  edgeRefs: { [key: dia.Cell.ID]: any }; // [link.id]: connRef
  ...
  constructor(graph: dia.Graph, options: IAvoidOptions) {
    ...
    this.avoidRouter = this.createAvoidRouter(options);
  }

  createAvoidRouter(options: IAvoidOptions) {
    const {
  ...
  static getAvoidRectFromElement(element: dia. Element) {
    const Avoid = AvoidLib.getInstance();
  ...
  updateConnector(link: dia.Link) {
    ...
    // );
    // Method does not exist in libavoid-js v4.
